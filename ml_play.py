...
class MLPlay:
    def __init__(self, side):
        ...
        if self.side == "1P":
            with open(os.path.join(os.path.dirname(__file__),'save','model_p1.pickle'), 'rb') as f:
                self.model = pickle.load(f)
        else:
            with open(os.path.join(os.path.dirname(__file__),'save','model_p2.pickle'), 'rb') as f:
                self.model = pickle.load(f)
    ...
    def update(self, scene_info):
        ...
        if not self.ball_served:
            ...
            if self.side == "1P":
                ...
                X = [scene_info["ball"][0], scene_info["ball"][1], scene_info["ball_speed"][0],scene_info["ball_speed"][1]]
